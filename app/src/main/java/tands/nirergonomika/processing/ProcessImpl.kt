...
class ProcessImpl(val context: Context) : Process {
    ...
    override fun process(wave: Wave): Float {
        ...
        val maximum = max(max(fingerprintSimilarityOne.score, fingerprintSimilarityTwo.score), fingerprintSimilarityThree.score)

        return if ( maximum < 1f) maximum else 1f
    }
}